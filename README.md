...
The ASTGDEMv2 is comrised of 22702 tiles covering land surfaces between **83°N** and **83°S**. Each tile is a **1°x1°** block of earch surface that contains at least 0.01% land area. Thus, tiles which contain only ocean area are missing. [Advantage: using 1°x1° tiles is much more efficient for geographic data processing. For example, covering the main area of London requires only two tiles.]

The ASTGDEMv2 is distributed in **GeoTIFF (.tif)** image/data format. The data are posted on a **1 arc-second (1"~30m at the equator)** grid. It should be noted that the size of each tile in the ASTGDEMv2 is in the zis



and referenced in **WGS-84 GCS**. In this case, the [*column*, *row*] of DEM image/data array represents the [*longitude*, *latitude*] of a specific location on the earth. 
